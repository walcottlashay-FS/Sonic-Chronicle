...
export default function PlaylistPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (const track of tracks) {
        const duplicate = uniqueTracks.some(
  ...
  async function handleCreatePlaylist(event) {
    ...
    if (selectedTracks.length === 0) {
      setMessage("Select at least one track from your Chronicle.");
      return;
    }
    ...
    try {
      ...
      setSpotifyUrl(result.data.spotifyUrl);
      setMessage("Your Spotify playlist was created.");
      setSelectedTracks([]);
      localStorage.removeItem("sonicPlaylistTracks");
    } catch (error) {
  ...
  return (
    <main>
      <p className="eyebrow">BUILD YOUR SOUNDTRACK</p>
      <h1>Create Playlist</h1>
      <p className="intro">
        Turn the tracks from your Chronicle into a Spotify playlist.
      </p>
      ...
      <nav className="navigation" aria-label="Main navigation">
        ...
        <Link className="button active-page" to="/playlist">
          Build Playlist
        </Link>
      </nav>
      ...
      <section className="page-section">
        ...
        {selectedTracks.length === 0 && !spotifyUrl && (
          <div className="playlist-empty">
            <p>No tracks are selected yet.</p>
        ...
        >
          ...
          >
            {loading ? "Creating Playlist..." : "Create on Spotify"}
          </button>
        </form>
